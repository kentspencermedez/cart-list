...
function displayList(list) {
  ...
  list.forEach(function(item, index) {
    ...
    itemPrice.textContent = `Price: ${Number(item.price).toLocaleString("en-PH", {
      style: "currency",
      currency: "PHP"
    })}`
  ...
  cartItems.textContent = (list.length > 1 || list.length === 0) ? `${list.length} Items` : `${list.length} Item`
...
sortButton.onchange = function(event) {
  ...
  if (selection === "subtotal-low-high") {
